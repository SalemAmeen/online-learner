...
local function process()
   ...
   if state.autolearn then
      for _,res in ipairs(state.results) do
         ...
         local cx = math.min(math.max(res.cx-off_x-1,0)/downs/encoder_dw+1,state.yuvFrame:size(3))
         local cy = math.min(math.max(res.cy-off_y-1,0)/downs/encoder_dw+1,state.yuvFrame:size(2))
